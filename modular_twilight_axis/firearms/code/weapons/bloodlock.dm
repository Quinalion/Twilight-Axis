...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/rifle/twilight_bloodlock/examine(mob/living/carbon/human/user)
	...
	if(ishuman(user))
		...
		if(HAS_TRAIT(u, TRAIT_ARCYNE) || HAS_TRAIT(u, TRAIT_VAMPBITE) || (u == bloodlock_owner) || is_blood_raider(u))
			if(cocked)
				if(chambered)
		...
		if(ishuman(user))
			...
			if(HAS_TRAIT(H, TRAIT_ARCYNE) && HAS_TRAIT(H, TRAIT_VAMPBITE))
				if(H.bloodpool < vitae_cost)
					...
					if(prob(5))
						to_chat(H, "<span style='color:#5E2129'>The bloodlock</span> slurs, " + span_cult(pick(
							"<i>\"Мне не хватает твоей крови. Найди кого-нибудь!\"</i>",
							"<i>\"Мне нужно больше крови.\"</i>",
							"<i>\"Ну же, забери чью-нибудь кровь для меня!\"</i>",
							"<i>\"Мне нечем запитаться...\"</i>"
						)))
					return
				to_chat(H, span_info("Оружие начинает вибрировать и запитываться..."))
				if(prob(5))
					to_chat(H, "<span style='color:#5E2129'>The bloodlock</span> slurs, " + span_cult(pick(
						"<i>\"Да, да, ДА! Какое же блаженство...\"</i>",
						"<i>\"Этот прилив сил... Пристрели шавку!\"</i>",
						"<i>\"Они поплатятся за то, что подняли на тебя клинок!\"</i>"
					)))
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/rifle/twilight_bloodlock/equipped(mob/user, slot)
	...
	if(is_blood_raider(H))
		if(prob(10))
			...
			"<i>\"Приветствую!\"</i>",
			"<i>\"Я скучал...\"</i>",
			"<i>\"Направь меня. Дай выстрелить!\"</i>"
		)))
		return
	...
	if(bloodlock_awakened)
		if(H == bloodlock_owner)
			reset_loss_timer()
			if(prob(10))
				to_chat(H, "<span style='color:#5E2129'>The bloodlock rifle</span> slurs, " + span_cult(pick(
					"<i>\"Здравствуй...\"</i>",
					"<i>\"Приветствую!\"</i>",
					"<i>\"Я скучал...\"</i>",
					"<i>\"Направь меня. Дай выстрелить!\"</i>"
	...
	to_chat(H, span_info(pick(
		"Вы чувствуете дискомфорт. Оружие будто пытается с вами связаться...",
		"Оружие едва заметно пульсирует под вашими пальцами.",
		"Что-то внутри оружия откликается на ваше присутствие.",
		"На мгновение кажется, будто оружие смотрит прямо на вас.",
		"Чужая воля касается вашего разума.",
		"Вы ощущаете слабый зов, исходящий от оружия."
	)))
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/rifle/twilight_bloodlock/proc/awaken_bloodlock(mob/living/carbon/human/H)
	...
	to_chat(H, "<span style='color:#5E2129'>The bloodlock rifle</span> slurs, " + span_cult(pick(
		"<i>\"Приветствую нового владельца...\"</i>",
		"<i>\"Ощущаешь меня?\"</i>",
		"<i>\"Почувствуй мой дар!\"</i>",
		"<i>\"Моё имя — С'анг. Запомни его.\"</i>",
		"<i>\"Прошлый владелец был противен мне... Но ты...!\"</i>"
	)))
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/rifle/twilight_bloodlock/proc/bloodlock_random_phrase()
	if(ishuman(src.loc))
		...
		if(prob(40))
			to_chat(H, "<span style='color:#5E2129'>The bloodlock rifle</span> whispers, " + span_cult(pick(
				"<i>\"Ещё когда меня держали в каком-то закоулке Мензоберразана, в лавку пришла группа Антракса. Помню лишь... тьму.\"</i>",
				"<i>\"Ходили слухи, что те, кто заточил меня и создал одну из первых оболочек, заключили какую-то сделку с тварями Инферно... А что думаешь ты?\"</i>",
				"<i>\"Кажется, запах железа застал тебя врасплох. Ты точно мне подходишь?!\"</i>",
				"<i>\"Дом Бэнр — властители Мензоберранзана. Младшая дочь заняла трон, назначив старших сестру и брата советниками... Ах, маги тесно связаны с призывами демонов. Как думаешь, тот огромный паук был его рук делом?\"</i>",
				"<i>\"Ох, вспоминаю прекрасные моменты в доме Грейвс. Как же часто меня тогда заводили, как же часто мы стреляли. Нет-нет, ты не подумай ничего плохого! Но ты и впрямь не дотягиваешь!\"</i>",
				"<i>\"Вон, посмотри на того урода. Может, выстрелим?\"</i>",
				"<i>\"Нихрена не вижу... Может, поднимешь ствол?\"</i>",
				"<i>\"Я думал, ты намного хуже, когда впервые меня подобрал. Приятно ошибаться.\"</i>",
				"<i>\"Когда мы уже что-нибудь сделаем?!\"</i>",
				"<i>\"Таким красавцем я стал не так уж давно. Может... пару йолей назад. Раньше меня так и водили по сосудам.\"</i>",
				"<i>\"Они назвали меня С'анг. Был ещё один... Или даже близнецы. Но их имён я совсем не помню.\"</i>",
				"<i>\"Пум-пурум-пурум-пум...\"</i>",
				"<i>\"А какой сейчас йол? Наверное, уже наступил тысячный, да?\"</i>",
				"<i>\"А... А?! Мьерда, ты тут...\"</i>"
			)))
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/rifle/twilight_bloodlock/proc/remove_bloodlock_power()
	...
	to_chat(H, "<span style='color:#5E2129'>The bloodlock rifle</span> slurs, " + span_cult(pick(
		"<i>\"Как ты посмел меня бросить?!\"</i>",
		"<i>\"Прошлый владелец всё же был лучше.\"</i>",
		"<i>\"Н'вах!\"</i>",
		"<i>\"Прогресс явно не для такого остолопа, как ты!\"</i>"
	)))
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/rifle/twilight_bloodlock/proc/bloodlock_second_examine(mob/living/carbon/human/H)
	...
	if(prob(50))
		if(prob(50))
			to_chat(H, "<span style='color:#5E2129'>The bloodlock</span> whispers, " + span_cult(pick(
				"<i>\"Какого Зизо?! Фальшивка!\"</i>",
				"<i>\"Нон... нон... НОН! УБЕЙ ВЛАДЕЛЬЦА ЭТОЙ ФАЛЬШИВКИ И УНИЧТОЖЬ ЕЁ!\"</i>",
				"<i>\"Пристрели владельца и уничтожь фальшивку, пока они нас не заметили!\"</i>",
				"<i>\"УБЕЙ, УБЕЙ, УБЕЙ!!\"</i>",
				"<i>\"Уничтожь самозванцев!\"</i>",
				"<i>\"Ха... Нам стоит покончить с фальшивкой, пока она не сделала это первой!\"</i>"
			)))
